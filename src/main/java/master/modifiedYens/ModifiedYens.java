...
public final class ModifiedYens extends Algorithm<PathFindingResult> {
    ...
    /**
     * Configure ModifiedYens to compute all source-target path under length l.
     */
    public static ModifiedYens sourceTarget(
            Graph graph,
    ...
    public PathFindingResult compute() {
        progressTracker.logDebug("Began yens");
        ...
        longestPathLength = shortestPath.get().relationshipIds().length;
        int i = 1;

        while (longestPathLength <= l) {
    ...
    private void addPathToSolution(
        ...
        if (pathToAdd.toPathResult().relationshipIds().length > longestPathLength) {
            longestPathLength = pathToAdd.toPathResult().relationshipIds().length;

            if (longestPathLength > l) {
                log.debug("breaking on length: " + longestPathLength);
                return;
            }
        }
    ...
    private Optional<PathResult> findFirstPath() {
        ...
        progressTracker.logInfo("progressTracker class " + progressTracker.getClass());
