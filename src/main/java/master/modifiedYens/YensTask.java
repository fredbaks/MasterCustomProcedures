...
public class YensTask implements Runnable {
    ...
    YensTask(
        ...
        this.relationshipFilterer = new RelationshipFilterer(1, trackRelationships);

        if (trackRelationships) {
            pathAppender = (rootPath, spurPath) -> rootPath.append(MutablePathResult.of(spurPath));
        } else {
            pathAppender = (rootPath, spurPath) -> rootPath
                    .appendWithoutRelationshipIds(MutablePathResult.of(spurPath));
        }
    }
